"""Query test provider for Snowflake backend.

Implements IQueryProvider for query feature tests using
...
class QueryProvider(IQuerySyncProvider, WorkerTestProtocol):
    ...
    def _reset_table_sync(self, model_class, table_name: str) -> None:
    ...
    async def _reset_table_async(self, model_class, table_name: str) -> None:
        from rhosocial.activerecord.backend.options import ExecutionOptions
        from rhosocial.activerecord.backend.schema import StatementType
    ...
    async def cleanup_after_test_async(self, scenario_name: str) -> None:
        tables_to_drop = [
            'comments', 'order_items', 'posts', 'orders', 'users',
            'json_users', 'nodes', 'extended_order_items', 'extended_orders',
    ...
    def _load_snowflake_schema(self, filename: str) -> str:
        schema_dir = os.path.join(os.path.dirname(__file__), "..", "rhosocial", "activerecord_snowflake_test", "feature", "query", "schema")
