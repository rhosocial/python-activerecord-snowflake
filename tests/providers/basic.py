...
package for the Snowflake backend.
...
class BasicProviderBase:
    ...
    def _load_snowflake_schema(self, filename: str) -> str:
        schema_dir = os.path.join(
            os.path.dirname(__file__), "..", "rhosocial", "activerecord_snowflake_test", "feature", "basic", "schema"
        )
...
class BasicAsyncProvider(BasicProviderBase, IBasicAsyncProvider):
    ...
    async def setup_type_test_model(self, scenario_name: str) -> Type[ActiveRecord]:
        ...
        from rhosocial.activerecord.backend.impl.snowflake import AsyncSnowflakeBackend
        _, config = get_scenario(scenario_name)
        temp_backend = AsyncSnowflakeBackend(connection_config=config)
