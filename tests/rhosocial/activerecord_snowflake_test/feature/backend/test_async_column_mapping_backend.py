# tests/rhosocial/activerecord_snowflake_test/feature/backend/test_async_column_mapping_backend.py
import pytest
...
async def setup_mapped_users_table(async_snowflake_backend):
    """Async fixture to create and drop a 'mapped_users' table for MySQL."""
    await async_snowflake_backend.execute("DROP TABLE IF EXISTS mapped_users")
...
@pytest.mark.asyncio
async def test_async_insert_with_mapping(async_snowflake_backend, setup_mapped_users_table):
    """
    Tests that async execute() with an INSERT correctly handles mapped data.
    """
    backend = async_snowflake_backend
    now = datetime.now()
...
@pytest.mark.asyncio
async def test_async_update_with_backend(async_snowflake_backend, setup_mapped_users_table):
    """
...
@pytest.mark.asyncio
async def test_async_fetch_with_combined_mapping_and_adapters(async_snowflake_backend, setup_mapped_users_table):
    """
