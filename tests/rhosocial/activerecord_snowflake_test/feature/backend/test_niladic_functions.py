# tests/rhosocial/activerecord_snowflake_test/feature/backend/test_niladic_functions.py
"""
...
class TestMySQLNiladicSelectContext:
    ...
    def test_select_current_timestamp_niladic(self, snowflake_backend):
        """SELECT CURRENT_TIMESTAMP (niladic, no parentheses) works in MySQL."""
        query = QueryExpression(
            dialect=snowflake_backend.dialect,
            select=[current_timestamp(snowflake_backend.dialect)],
        )
        sql, params = query.to_sql()
        assert sql == "SELECT CURRENT_TIMESTAMP"
        assert "(" not in sql.replace("SELECT ", "")
        result = snowflake_backend.execute(sql, params, options=DQL_OPTIONS)
        assert result.data is not None
        assert len(result.data) == 1

    def test_select_current_timestamp_with_parens(self, snowflake_backend):
        """SELECT CURRENT_TIMESTAMP() (with parentheses) also works in MySQL."""
        query = QueryExpression(
            dialect=snowflake_backend.dialect,
            select=[FunctionCall(snowflake_backend.dialect, 'CURRENT_TIMESTAMP')],
        )
        sql, params = query.to_sql()
    ...
    def test_select_current_date_niladic(self, snowflake_backend):
        """SELECT CURRENT_DATE (niladic) works in MySQL."""
        query = QueryExpression(
            dialect=snowflake_backend.dialect,
            select=[current_date(snowflake_backend.dialect)],
        )
        sql, params = query.to_sql()
    ...
    def test_select_current_time_niladic(self, snowflake_backend):
        """SELECT CURRENT_TIME (niladic) works in MySQL."""
        query = QueryExpression(
            dialect=snowflake_backend.dialect,
            select=[current_time(snowflake_backend.dialect)],
        )
        sql, params = query.to_sql()
    ...
    def test_select_now(self, snowflake_backend):
        """SELECT NOW() (regular function, always with parentheses) works in MySQL."""
        query = QueryExpression(
            dialect=snowflake_backend.dialect,
            select=[now(snowflake_backend.dialect)],
        )
        sql, params = query.to_sql()
        assert "NOW()" in sql
        result = snowflake_backend.execute(sql, params, options=DQL_OPTIONS)
...
class TestMySQLNiladicDDLContext:
    ...
    def test_ddl_default_current_timestamp_niladic(self, snowflake_backend):
        """DEFAULT CURRENT_TIMESTAMP (niladic) works in MySQL DDL."""
        dialect = snowflake_backend.dialect
        table_name = 'test_niladic_ddl_1'

        # Clean up
        snowflake_backend.execute(*DropTableExpression(
        ...
        try:
            ...
            cols = snowflake_backend.introspector.list_columns(table_name)
    ...
    def test_ddl_default_current_timestamp_with_parens(self, snowflake_backend):
    ...
    def test_ddl_default_current_timestamp_with_precision(self, snowflake_backend):
...
class TestAsyncMySQLNiladicSelectContext:
    ...
    @pytest.mark.asyncio
    async def test_async_select_current_timestamp_niladic(self, async_snowflake_backend):
        """SELECT CURRENT_TIMESTAMP (niladic) works in MySQL (async)."""
        query = QueryExpression(
            dialect=async_snowflake_backend.dialect,
            select=[current_timestamp(async_snowflake_backend.dialect)],
        )
        sql, params = query.to_sql()
        assert sql == "SELECT CURRENT_TIMESTAMP"
        result = await async_snowflake_backend.execute(sql, params, options=DQL_OPTIONS)
        assert result.data is not None
        assert len(result.data) == 1
...
class TestAsyncMySQLNiladicDDLContext:
    ...
    @pytest.mark.asyncio
    async def test_async_ddl_default_current_timestamp_niladic(self, async_snowflake_backend):
        """DEFAULT CURRENT_TIMESTAMP (niladic) works in MySQL DDL (async)."""
        dialect = async_snowflake_backend.dialect
        table_name = 'test_async_niladic_ddl_1'

        # Clean up
        await async_snowflake_backend.execute(*DropTableExpression(
            dialect=dialect, table=table_name, if_exists=True
        ).to_sql())
        ...
        try:
            ...
            cols = await async_snowflake_backend.introspector.list_columns(table_name)
            col_names = [c.name for c in cols]
            assert 'ts' in col_names
        finally:
            await async_snowflake_backend.execute(*DropTableExpression(
