# tests/rhosocial/activerecord_snowflake_test/feature/connection/conftest.py
"""
Pytest configuration for connection pool tests.

This module provides fixtures for testing connection pools with Snowflake backends.
"""
...
def create_mysql_backend_factory(config_dict: Dict[str, Any]):
    """Create a factory function that produces SnowflakeBackend instances."""

    def factory():
        config_copy = config_dict.copy()
        ssl_disabled = config_copy.pop("ssl_disabled", None)
        config = SnowflakeConnectionConfig(**config_copy)
...
def create_async_mysql_backend_factory(config_dict: Dict[str, Any]):
    """Create a factory function that produces AsyncSnowflakeBackend instances."""
...
def mysql_pool(request) -> Generator[BackendPool, None, None]:
    """Create a BackendPool with Snowflake backends for testing."""
...
async def async_mysql_pool(request) -> AsyncBackendPool:
    """Create an AsyncBackendPool with Snowflake backends for testing."""
    scenario_name = request.param
    config_dict = get_scenario_config(scenario_name).copy()
